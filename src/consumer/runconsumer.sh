...
log_path="./logs/consumer/${topic_title}/${CURRENT_DATE}/${CURRENT_TIME}/Pod_${pod_index}"
mkdir -p ${log_path}
...
#  echo topics are ${TOPICS[@]:$spoint:$num_topics}
  ...
  python3 mainConsumer.py  -topics "${TOPICS[@]:$spoint:$num_topics}" -topictitle "${topic_title}" -consindex "$i" -hpath "${h5pypath}" -colrange "${col_range}" -pi "${pod_index}" -uris "${server_uri}" >&"${log_path}/consumer.$i.$((nconsumers - 1)).out"  &
